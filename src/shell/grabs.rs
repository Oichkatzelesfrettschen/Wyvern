...
//!
//! Implements pointer grabs for interactive window resize and move.
//!
//! ## Resize State Machine
//!
//! Tracks resize progress through states:
//! - `NotResizing`: Window not being resized
//! - `Resizing`: Pointer button held, tracking mouse motion
//! - `WaitingForFinalAck`: Waiting for client to acknowledge final size
//! - `WaitingForCommit`: Waiting for client to commit new surface size
//!
//! ## Event Handling
//!
//! Processes:
//! - Pointer motion: Computes new window size and sends configure event
//! - Pointer button release: Transitions to `WaitingForFinalAck`
//! - Surface commit: Transitions to `NotResizing`
//!
//! Gracefully handles client protocol violations with logging.
                ...
                WindowSurface::Wayland(xdg) => {
                    ...
                    with_states(&self.window.wl_surface().unwrap(), |states| {
                        ...
                        match data.resize_state {
                            ...
                            _ => {
                                // Invalid resize state: likely race condition or client error.
                                // Log and skip state transition to prevent compositor crash.
                                tracing::warn!(
                                    state = ?data.resize_state,
                                    "configure_send_initial: unexpected resize state (expected Resizing)"
                                );
                            }
                        }
                    });
                    ...
                    with_states(&surface, |states| {
                        ...
                        match data.resize_state {
                            ...
                            _ => {
                                ...
                                tracing::warn!(
                                    state = ?data.resize_state,
                                    "configure_send_initial (X11): unexpected resize state (expected Resizing)"
                                );
                            }
                        }
                    });
                ...
                with_states(&self.window.wl_surface().unwrap(), |states| {
                    ...
                    match data.resize_state {
                        ...
                        _ => {
                            ...
                            tracing::warn!(
                                state = ?data.resize_state,
                                "motion: unexpected resize state (expected Resizing)"
                            );
                        }
                    }
                });
                ...
                with_states(&surface, |states| {
                    ...
                    match data.resize_state {
                        ...
                        _ => {
                            ...
                            tracing::warn!(
                                state = ?data.resize_state,
                                "motion (X11): unexpected resize state (expected Resizing)"
                            );
                        }
                    }
                });

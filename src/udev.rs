...
//!
//! Full-featured DRM (Direct Rendering Manager) backend using libdrm and GBM.
//!
//! Provides:
//! - Direct framebuffer access via DRM KMS (Kernel Mode Setting)
//! - GPU-accelerated rendering with GLES via EGL
//! - Multi-GPU support with primary GPU selection
//! - Direct scanout optimization for fullscreen surfaces
//! - 10-bit color format support (if enabled)
//!
//! ## Configuration
//!
//! - `WYVERN_DRM_DEVICE`: Select primary GPU (default: auto-detect)
//! - `WYVERN_DISABLE_10BIT`: Disable 10-bit color formats
//! - `WYVERN_DISABLE_DIRECT_SCANOUT`: Disable direct scanout optimization
//!
//! ## Running
//!
//! ```bash
...
pub fn run_udev(
    ...
    event_loop
        ...
        .insert_source(
            event_rx_main,
            move |event, _metadata, data: &mut WyvernState<UdevData>| {
...
impl WyvernState<UdevData> {
    fn device_added(&mut self, node: DrmNode, path: &Path) -> Result<(), DeviceAddError> {
        ...
        let mut try_initialize_gpu = || {
            // SAFETY: EGLDisplay::new requires FFI call to initialize EGL display from GBM device.
            // GBM device is cloned (Rc-like semantics), ensuring it outlives the EGL display.
            // This is a GPU device initialization in a DRM hotplug handler; thread-local context.
            ...
            Err(err) => {
                ...
                match err {
                    ...
                    SwapBuffersError::ContextLost(err) => {
                        // EGL context lost: likely due to GPU reset or mode switch.
                        // Log the error and skip rendering until context is recovered.
                        // TODO (Phase 1): Implement context recovery and re-initialization.
                        tracing::error!(
                            "EGL context lost: {}. Rendering paused until recovery.",
                            err
                        );
                        false
                    }
            ...
            Err(err) => {
                ...
                match err {
                    ...
                    SwapBuffersError::ContextLost(err) => match err.downcast_ref::<DrmError>() {
                        ...
                        _ => {
                            // Unexpected DrmError in ContextLost: log and skip rendering recovery.
                            // TODO (Phase 1): Implement context recovery for other error types.
                            tracing::error!(
                                "Unhandled DrmError in ContextLost: {}. Rendering paused.",
                                err
                            );
                            false
                        }

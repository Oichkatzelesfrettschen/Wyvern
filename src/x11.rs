...
//!
//! X11 server backend for compatibility with X11 desktop environments.
//!
//! Provides:
//! - Rendering to X11 windows via Xlib/X11 protocol
//! - GPU-accelerated rendering with OpenGL ES (Vulkan optional)
//! - Keyboard and pointer input from X11 events
//! - Multi-display support (Xinerama/RandR)
//!
//! Useful for:
//! - Testing on systems without full Wayland support
//! - Running alongside X11 applications
//! - Legacy environment compatibility
//!
//! ## Configuration
//!
//! - `WYVERN_NO_VULKAN`: Disable Vulkan, use OpenGL ES only
//!
//! ## Running
//!
//! ```bash
...
pub struct X11Data {
    render: bool,
    mode: Mode,
    // SAFETY: ManuallyDrop ensures GlesRenderer drops BEFORE X11Surface (via custom Drop impl).
    // This is critical because GlesRenderer needs the EGL context to be valid during cleanup
    // (MakeCurrent call), and X11Surface holds the GBM device that backs EGL.
    // Drop order: renderer (first) -> surface (second) -> other fields.
    renderer: ManuallyDrop<GlesRenderer>,
...
impl Drop for X11Data {
    fn drop(&mut self) {
        // SAFETY: ManuallyDrop allows explicit control over drop order.
        // We drop the renderer before the surface to ensure:
        // 1. GlesRenderer's cleanup code (MakeCurrent, etc.) runs while X11Surface is still alive
        // 2. The EGL context remains valid during renderer cleanup
        // 3. X11Surface and its GBM device are dropped after renderer is fully cleaned up
        unsafe {
            ManuallyDrop::drop(&mut self.renderer);
        }
        // Other fields (surface, dmabuf_state, etc.) drop in declaration order after renderer.
    }
}
...
pub fn run_x11(
    ...
    event_loop
        ...
        .insert_source(
            event_rx_main,
            move |event, _metadata, data: &mut WyvernState<X11Data>| {
                if let calloop::channel::Event::Msg(CustomEvent::TilingRecalculate) = event {
    ...
    // SAFETY: EGLDisplay::new requires FFI call to EGL platform initialization.
    // The device is cloned (Rc) and retained by this scope, ensuring device outlives the display.
    // Single-threaded event loop ensures no concurrent access to the EGL context.
    ...
    // SAFETY: GlesRenderer::new requires FFI call to initialize OpenGL ES context.
    // The EGL context is valid and bound to the current thread (single-threaded event loop).
    // The renderer takes ownership of the context reference and ensures safe cleanup on drop.
    let mut renderer =

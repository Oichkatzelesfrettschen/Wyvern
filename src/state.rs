...
//!
//! Central state management for the Wyvern compositor.
//!
//! [`WyvernState`] is the main event dispatch hub, managing:
//! - Client connections and surface trees
//! - Keyboard and pointer input
//! - Output (display) configuration
//! - Window focus and Z-ordering
//! - Rendering and damage tracking
//! - Protocol delegation (buffer, dmabuf, data device, etc.)
//!
//! ## Generic Backend Pattern
//!
//! `WyvernState<T>` is parameterized over backend-specific data `T`:
//! - `T = UdevData`: DRM/KMS rendering on Linux
//! - `T = WinitData`: Desktop window backend
//! - `T = X11Data`: X11 server rendering
//!
//! This allows shared compositor logic while supporting multiple rendering backends.
//!
//! ## Event Dispatch
//!
//! The event loop posts [`CustomEvent`] messages to the calloop event dispatcher,
//! which calls [`process_input_event`] and [`render`] to update compositor state.
...
use smithay::{
    ...
    reexports::{
        ...
        wayland_server::{
            ...
            protocol::{wl_buffer::WlBuffer, wl_data_source::WlDataSource, wl_surface::WlSurface},
...
use crate::{
    focus::{FocusStack, KeyboardFocusTarget, PointerFocusTarget},
    shell::WindowElement,
};
#[cfg(feature = "xwayland")]
use smithay::{
...
impl<BackendData: Backend + 'static> WyvernState<BackendData> {
    pub fn recalculate_tiling(&mut self) {
        ...
        for output in self.space.outputs().cloned().collect::<Vec<_>>() {
            ...
            let mut tiled_windows: Vec<WindowElement> = self
                ...
                .filter(|window| {
                    self.space.outputs_for_element(window).contains(&output)
                        && !window.0.toplevel().is_some_and(|t| {
                            t.current_state()
                                .states
            ...
            let total_gaps_width = if tiled_windows.len() > 1 {
                gap * 3
            } else {
                gap * 2
            }; // Left, right, and middle gap for master-stack, or just left/right for single
            let total_gaps_height = gap * 2; // Top/bottom gap (same for all layouts)
            ...
            if tiled_windows.len() == 1 {
                if let Some(toplevel) = tiled_windows[0].0.toplevel() {
                    ...
                    self.space
                        .map_element(tiled_windows[0].clone(), window_geometry.loc, false);
                ...
                if let Some(toplevel) = tiled_windows[0].0.toplevel() {
                    ...
                    self.space
                        .map_element(tiled_windows[0].clone(), master_geometry.loc, false);
                    toplevel.send_configure();
                }
                ...
                let stack_height_per_window =
                    (tiled_area_height - (num_stack_windows - 1) * gap) / num_stack_windows;
                ...
                for (i, window) in stack_windows.iter().enumerate() {
                    let stack_geometry = Rectangle::new(
                        smithay::utils::Point::from((
                            ...
                            output_geometry.loc.y
                                + gap
                                + i as i32 * (stack_height_per_window + gap),
                        )),
                    ...
                    if let Some(toplevel) = window.0.toplevel() {
                        ...
                        self.space
                            .map_element(window.clone(), stack_geometry.loc, false);
                        toplevel.send_configure();
                    }
                }
            }
        ...
        handle
            .insert_source(
                ...
                |_, display, data| {
                    ...
                    // SAFETY: get_mut requires unsafe because calloop::Generic holds display as opaque pointer.
                    // The display is not dropped in this scope (only mutated via dispatch_clients).
                    // Single-threaded event loop ensures exclusive access to display during dispatch.
                    unsafe {
